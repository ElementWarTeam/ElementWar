...
public class AnalogJoystickComponent: SKSpriteNode {
    ...
    private func redrawTexture() {
        ...
        let rectPath = UIBezierPath(ovalInRect: CGRect(origin: CGPoint.zero, size: needSize))
        rectPath.addClip()
        ...
        if let img = image {
            img.drawInRect(CGRect(origin: CGPoint.zero, size: needSize), blendMode: .Normal, alpha: 1)
        }
